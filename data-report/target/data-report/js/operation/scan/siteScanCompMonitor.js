function querySiteScanCompMonitor(){
	...
	if($(".selType").is(':checked')){
		var ewbNo = $("input[name='billNo']").val();
		if(!ewbNo){
			$(".loadDiv").hide();
			alert("请输入单号");
			return false;
		...
		reqParams={
			'ewbNo' : $("input[name='billNo']").val(),
			'compType' : $("#compTypeId option:selected").val(),
			...
			reqParams = {
					...
					'scanSite' : $("input[name='siteId']").val(),
					'compType' : $("#compTypeId option:selected").val(),
					'pageSize' : $(".pageSize").val(),
					'pageNum' : $(".pageNum").val()
				};
	...
	$.ajax({
		type : "POST",
		async : false,
		url : "querySiteScanCompMonitorByPage",
		data : reqParams ,
		dataType : "text",
			...
			if(pageEntity.error == false){
				...
				if(dataList.length > 0){
					for (var i = 0; i < dataList.length; i++) {
						htm+="<tr><td>"+(i+1)+"</td><td>"+dataList[i].ewbNo+"</td><td>"+dataList[i].scanSite+"</td>";
						htm+="<td>"+dataList[i].scanTime+"</td><td>"+dataList[i].scanType+"</td><td>"+dataList[i].createdBy+"</td>";
					}
					$("#tbody_siteScanCompMonitorDisplayId").html(htm);
					$(".listTable tr th").each(function(index){
						$(this).width($("#tbody_siteScanCompMonitorDisplayId tr td").eq(index).width());
					});
				}else{
					$("#tbody_siteScanCompMonitorDisplayId").html('<tr><td colspan ="6"><center style="color:red">未查询到数据</center></td></tr>');
				}
