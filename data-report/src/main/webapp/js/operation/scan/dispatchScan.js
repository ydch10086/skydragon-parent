				...
				if(dataList.length > 0){
					for (var i = 0; i < dataList.length; i++) {
						var num = i+1;
								...
								htm += "<td>" + dataList[i].createdBy + "</td>"
										+ "<td>" + dataList[i].scanTime
										+ "</td>" + "<td>"
										+ dataList[i].deliveryMan + "</td>";
								htm += "<td>" + dataList[i].dictName + "</td>";
					...
					$("#tbody_dispatchScanDisplayId").html(htm);
					$(".listTable tr th").each(function(index){
						$(this).width($("#tbody_dispatchScanDisplayId tr td").eq(index).width());
					});
				}else{
					$("#tbody_dispatchScanDisplayId").html('<tr><td colspan ="9"><center style="color:red">未查询到数据</center></td></tr>');
				}
